...
import numpy as np
import requests
from astropy import coordinates as coord, time, units as u
...
class CatalogueClient:
    ...
    def ecef_positions(
        ...
        for tle in tles:
            ...
            if e != 0:
                continue  # skip propagation failures
    ...
    def celestial_positions(
        ...
        Returns a list of dicts with keys: name, ra_hours, dec_degrees, distance_km.
        """
        tles = self.fetch_tles(dt)
        if dt is None:
            dt = datetime.datetime.now(datetime.timezone.utc)
        ...
        for tle in tles:
            ...
            # TEME -> ICRS (RA/Dec)
            icrs = teme.transform_to(coord.ICRS())
            ...
            results.append({
                ...
                "distance_km": round(
                    np.sqrt(pos[0]**2 + pos[1]**2 + pos[2]**2), 1
                ),
            })
